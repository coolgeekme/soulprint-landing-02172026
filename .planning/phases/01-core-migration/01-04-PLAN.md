...
<task type="auto">
  ...
  <action>
Update app/import/page.tsx to call /api/import/trigger and poll for progress updates using useRef for proper cleanup.

**Changes needed:**

1. **Add state and ref for polling interval** (BLOCKER 2 FIX):
...
2. **After chunked upload completes**, replace queue-processing call with trigger call:
...
3. **Add progress polling function with useRef storage** (BLOCKER 2 FIX):
...
4. **Add useEffect for cleanup on unmount** (BLOCKER 2 FIX):
...
**Important:**
- Use useRef to store interval ID (BLOCKER 2 FIX - provides cleanup access in useEffect)
...
- Clean up interval in useEffect on unmount (prevents memory leaks)
  ...
  <verify>
Check that import page calls /api/import/trigger and has useRef + useEffect cleanup:
```bash
grep -E "import/trigger|progress_percent|import_stage" /home/drewpullen/clawd/soulprint-landing/app/import/page.tsx
grep -E "useRef.*Timeout|pollingIntervalRef" /home/drewpullen/clawd/soulprint-landing/app/import/page.tsx || echo "WARNING: Missing useRef for interval ID"
grep -E "useEffect.*return.*clearInterval.*pollingIntervalRef" /home/drewpullen/clawd/soulprint-landing/app/import/page.tsx || echo "WARNING: Missing useEffect cleanup with useRef"
```
  </verify>
  <done>Frontend updated to call /api/import/trigger, stores polling interval ID in useRef, polls progress_percent and import_stage every 2s, shows progress bar with stage messages, includes useEffect cleanup using pollingIntervalRef to prevent memory leaks</done>
</task>
...
3. Frontend stores polling interval ID in useRef (pollingIntervalRef)
4. Frontend polls user_profiles for progress_percent and import_stage
5. Progress UI shows percentage and stage messages
6. Polling stops when import_status becomes 'quick_ready' or 'failed'
7. useEffect cleanup wrapper properly clears polling interval using pollingIntervalRef on unmount
8. Vercel endpoint maxDuration is 30s (not 300s like old process-server)
</verification>
...
- No memory leaks from polling interval (useRef + useEffect cleanup verified)
