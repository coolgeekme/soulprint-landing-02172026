...
3. **Streaming JSON parsing from file with format detection** (Research Pattern 1 + wrapped format fallback):
```python
    ...
    with open(file_path, 'rb') as f:
        try:
            ...
            try:
                ...
                # Both formats failed, return empty list
                pass
            ...
            async with httpx.AsyncClient() as client:
                await client.patch(
                    ...
                    json={
                        ...
                        "import_error": "Quick pass generation failed",
                        "progress_percent": 100,
                        "import_stage": "Failed"
                    },
                    headers={
...
- **BLOCKER 3 FIX:** Parse function tries bare array first, then wrapped format on failure
- Clean up temp file in `finally` block
- Update progress at EVERY stage (0%, 20%, 50%, 100%)
- **BLOCKER 1 FIX:** Set import_status to 'failed' when quick_pass_result is None (not 'quick_ready')
...
```
  ...
  <done>streaming_import.py created with update_progress, download_streaming (to temp file), parse_conversations_streaming (from temp file with wrapped format detection), and process_import_streaming using true constant-memory streaming via temporary file, setting import_status to 'failed' when quick pass fails</done>
</task>
...
<verification>
1. streaming_import.py exists with complete pipeline (download to temp file, parse from temp file with format detection, quick pass, progress updates)
...
5. Parser handles both bare array [...] and wrapped { conversations: [...] } formats
6. Progress updates at 0%, 20%, 50%, 100% with stage messages
7. Error handling updates import_status to 'failed' with specific error message
8. Quick pass failure sets import_status to 'failed' (not 'quick_ready')
9. Temp file cleaned up in finally block
</verification>
...
- Parser detects and handles both bare array and wrapped formats
...
- Quick pass failure sets import_status to 'failed' with error message (allows retry)
