...
must_haves:
  ...
  key_links:
    ...
    - from: "scripts/eval-memory-quality.ts"
      to: "user_profiles (Supabase)"
      via: "SHA256 reverse lookup: fetch all user_ids, hash each, match against dataset user_id_hash to get real user_id for memory_md/chunks query"
      pattern: "createHash.*sha256|user_id_hash"
---
  ...
  <action>
    ...
    **Part B: Create `scripts/eval-memory-quality.ts`**
    ...
    3. Fetch memory data for users in the dataset:
       ...
       - Build a hash-to-real-id reverse lookup: fetch ALL user_profiles (just `user_id, memory_md` columns), hash each user_id with SHA256 (same `createHash('sha256').update(userId).digest('hex')` as in datasets.ts), build a `Map<hash, user_id>`. This works because the user base is small (dozens, not millions).
       - For each matched user, fetch their memory_md and a sample of conversation_chunks from Supabase using the REAL user_id
       - Build a memory_context string from chunks (top 5 chunks concatenated)
       - Log how many dataset users were matched vs unmatched. If zero matched, print helpful error and exit.
