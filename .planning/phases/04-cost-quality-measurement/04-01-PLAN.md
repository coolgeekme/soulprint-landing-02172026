...
user_setup:
  - service: supabase
    why: "import_cost_json column must exist in user_profiles before pipeline can save cost data"
    dashboard_config:
      - task: "Add import_cost_json TEXT column to user_profiles table"
        location: "Supabase Dashboard -> SQL Editor -> Run: ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS import_cost_json TEXT;"
...
<task type="auto">
  ...
  <action>
    ...
    **full_pass.py:**
    ...
    PREREQUISITE: The `import_cost_json` TEXT column must exist in user_profiles before deploying. This is declared in user_setup frontmatter -- the execute-plan workflow will prompt the user to run the ALTER TABLE command before execution begins. The upsert goes through update_user_profile() which has error handling, but the column MUST exist for data to actually persist.
  </action>
  <verify>
    ...
    10. Verify user_setup prerequisite is documented: grep confirms `import_cost_json` column referenced in user_setup frontmatter of this plan
  </verify>
  <done>
...
3. CostTracker correctly computes costs: `python3 -c "from processors.cost_tracker import CostTracker; t = CostTracker(); t.llm_input_tokens = 100000; t.llm_output_tokens = 10000; s = t.get_summary(); assert abs(s['llm_cost_usd'] - 0.15) < 0.001, f'Expected ~0.15 got {s[\"llm_cost_usd\"]}'; print('Cost computation OK')"` (run from rlm-service/)
