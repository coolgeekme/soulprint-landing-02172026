...
3. Create v1 prompt builder that replicates the current `buildSystemPrompt` logic from `app/api/chat/route.ts`.
   ...
   **Approach: Copy the prompt construction logic inline** (do NOT extract a shared helper from chat/route.ts). The chat route's `buildSystemPrompt` is a private inline function that takes a `UserProfile` plus runtime parameters (dailyMemory, memoryContext, isOwner, forcedSearchContext). The baseline only needs the owner-mode prompt construction with soulprint sections. Copying inline avoids modifying the chat route (which is out of scope for this plan) and avoids coupling the eval system to the chat route's signature.

   **Specifically, the v1 prompt builder function must:**
   - Accept a `ChatEvalItem` and return a `string`
   - Start with the same preamble: `# SoulPrint\n\nYou have memories of this person...` (copy the exact text from lines 605-613 of `app/api/chat/route.ts`)
   - Include date/time using the same format as the chat route (`toLocaleDateString` with weekday/year/month/day)
   - Call `cleanSection()` on each section from `item.soulprint_context` (soul, identity, user, agents, tools)
   - Call `formatSection('SOUL', soul)` etc. for each section, appending non-null results to the prompt
   - Use AI name "SoulPrint" (hardcoded, since eval doesn't have per-user names)
   - Skip dailyMemory, memoryContext, forcedSearchContext, and imposter mode (not relevant for baseline evaluation)

   **Why copy instead of extract:** The chat route will be refactored in Phase 2 (Prompt Template System) where the v2 prompt builder replaces v1. Extracting a shared helper now would create coupling that Phase 2 would immediately need to undo. The baseline is meant to freeze a snapshot of v1 behavior for comparison.
...
- Implements `--help` flag: if `process.argv.includes('--help')`, print the usage string and call `process.exit(0)` before any other logic runs
...
**Environment loading:** Each script should load `.env.local` for Supabase, Bedrock, and Opik credentials. Use `import 'dotenv/config'` with `DOTENV_CONFIG_PATH=.env.local` or use the tsx `--env-file` flag. Document the correct invocation in a comment at the top of each script.
  ...
  <verify>
    ...
    Runtime argument parsing check — run each script with `--help` flag and confirm it exits cleanly with usage text (not a crash or unhandled error):
      `npx tsx scripts/create-eval-dataset.ts --help` → exits 0, prints usage
      `npx tsx scripts/run-experiment.ts --help` → exits 0, prints usage
      `npx tsx scripts/record-baseline.ts --help` → exits 0, prints usage
    Each script MUST implement a `--help` handler that prints the usage string from the action section and calls `process.exit(0)`.
  </verify>
  <done>
...
- NOTE: Phase 1 Success Criterion #5 (Opik latency <100ms P95) is deferred to Phase 5 (Integration Validation). Phase 5 SC#3 explicitly covers "Async observability adds <100ms P95 latency overhead under 100 concurrent requests" with proper load testing. Phase 1 builds offline evaluation infrastructure, not production latency benchmarks.
</success_criteria>
