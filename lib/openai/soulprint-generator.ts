import { generateContent, ChatMessage } from './client';
import type { QuestionnaireAnswers, SoulPrintData } from '@/lib/gemini/types';

/**
 * SoulPrint Generator using OpenAI GPT-4o
 * Ported from the Gemini implementation
 */
...
/**
 * Generate a SoulPrint from questionnaire answers using OpenAI GPT-4o
 */
export async function generateSoulPrint(answers: QuestionnaireAnswers, userId?: string): Promise<SoulPrintData> {
    const startTime = Date.now();
    console.log('🔮 [OpenAI Generator] Starting SoulPrint generation sequence...');
    ...
    for (let i = 0; i < baseAttempts.length; i++) {
        ...
        console.log(`📡 [OpenAI Generator] Attempt ${i + 1}: Generating base JSON (temp: ${attempt.temperature})...`);
        ...
        try {
            ...
            baseSoulprint = JSON.parse(response);
            console.log(`✅ [OpenAI Generator] Base JSON generated in ${baseDuration.toFixed(2)}s`);
            break;
        } catch (error) {
            console.error(`❌ [OpenAI Generator] Failed to parse base SoulPrint JSON on attempt ${i + 1}:`, error);
        }
    }
    ...
    console.log('📡 [OpenAI Generator] Phase 2: Generating full system prompt (500+ words)...');
...
${JSON.stringify(baseSoulprint, null, 2)}`;
    ...
    const systemPromptMessages: ChatMessage[] = [
        { role: 'system', content: SOULPRINT_SYSTEM_PROMPT },
        { role: 'user', content: systemPromptRequest }
    ];

    const fullSystemPrompt = await generateContent(systemPromptMessages, {
        temperature: 0.4,
        maxTokens: 4096
    });
    ...
    if (!fullSystemPrompt || fullSystemPrompt.length < 100) {
        console.warn('⚠️ [OpenAI Generator] Full system prompt generation returned suspiciously short content');
    } else {
        console.log(`✅ [OpenAI Generator] Full system prompt generated in ${fullPromptDuration.toFixed(2)}s (${fullSystemPrompt.length} chars)`);
    }

    baseSoulprint.full_system_prompt = fullSystemPrompt.trim();

    const totalDuration = (Date.now() - startTime) / 1000;
    console.log(`✨ [OpenAI Generator] SoulPrint sequence complete. Total time: ${totalDuration.toFixed(2)}s`);

    return baseSoulprint as SoulPrintData;
}

/**
 * Convert SoulPrint data to a text document for indexing
 */
